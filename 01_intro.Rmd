...
```{r, echo = FALSE}
GENDER <- kommrep_loc %>%
  select(v_exit, v_geschlecht, weight_ps) %>%
...
RACE <- kommrep_loc_lm %>%
  select(v_exit, racialized, weight_ps) %>%
  mutate(exit = as.character(as_factor(v_exit))) %>%
  mutate(exit = recode(
    exit,
    `Ja` = "yes",
...
```{r, echo = FALSE}
GENDER <- kommrep_loc %>%
  select(v_sorge_umgang4, v_geschlecht, weight_ps) %>%
  ...
  mutate(gender = factor(gender, levels = c("male", "female/diverse"))) %>%
  select(gender, `avoid topics`, weight_ps) %>%
...
RACE <- kommrep_loc_lm %>%
  select(v_sorge_umgang4, racialized, weight_ps) %>%
  mutate(`avoid topics` = ifelse(v_sorge_umgang4 == 1L, TRUE, FALSE)) %>%
  mutate(`avoid topics` = as.character(as_factor(v_sorge_umgang4))) %>%
  mutate(`avoid topics` = recode(
    `avoid topics`,
...
CLASS <- kommrep_loc %>%
  select(v_sorge_umgang4, v_schicht, weight_ps) %>%
  mutate(`avoid topics` = ifelse(v_sorge_umgang4 == 1L, TRUE, FALSE)) %>%
  mutate(`avoid topics` = as.character(as_factor(v_sorge_umgang4))) %>%
  mutate(`avoid topics` = recode(
    `avoid topics`,
  ...
  filter(!is.na(class)) %>%
  mutate(`class` = droplevels(`class`)) %>%
  select(`avoid topics`, `class`, weight_ps) %>%
