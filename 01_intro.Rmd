...
threat_physical_index <- lapply(
  # v_perzb_art3 = Art der der Bedrohung: direkter Begegnung
  # v_perzb_art4 = Art der der Bedrohung: Sachbeschädigung
  # v_perzb_art5 = Art der der Bedrohung: Körperlich bedrängt oder angegriffen
  paste0("v_perzb_art", 4:5),
  function(var){
    x <- as.vector(kommrep_loc[[var]])
    x <- ifelse(x == 98, NA, x)
    x <- x - 1
...
kommrep_loc_lm <- kommrep_loc %>%
  ...
  mutate(`kommunikative Inzivilität` = ifelse(
    is.na(`kommunikative Inzivilität`),
    FALSE,
    `kommunikative Inzivilität`)
  ) %>% 
  
  mutate(threat_physical = threat_physical_index / 2) %>% 
  ...
  mutate(`physische Inzivilität` = ifelse(
    is.na(`physische Inzivilität`),
    FALSE,
...
exit1 <- glm(
  Rückzugsgedanke ~ `kommunikative Inzivilität` + threat_physical_bin,
  # Rückzugsgedanke ~ threat_verbal + threat_physical,
